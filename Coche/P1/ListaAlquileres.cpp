...
ListaAlquileres::ListaAlquileres(const ListaAlquileres& otro): cont(otro.cont), nElems(otro.nElems)
{
	alquileres = new Alquiler*[nElems];
	for (int i = 0; i < otro.cont; i++) 
	{
		if(otro.alquileres[i]->getCoche() != nullptr)
		alquileres[i]->setCoche(otro.alquileres[i]->getCoche());
	}
}
...
void ListaAlquileres::insertaAlquiler(Alquiler& a, ListaCoches& listaCoches) 
{
	...
	else
	{
		...
		else if (i < cont)
		{
			...
			if (indiceCoche == -1) 
			{
				alquileres[i]->setCocheValido(false);
			}
			else 
			{
