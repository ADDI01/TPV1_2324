...
Cannon::Cannon(Point2D<float> pos, Texture* texture, pair<uint, uint> size, GameState* State, uint nLifes, float shootCD,
	float velocity, PlayState* myPlayState): SceneObject(State, myPlayState, texture, pos, size, nLifes), _shootCD(shootCD), _velocity(velocity),
	_myRect(SDL_Rect()) {};

Cannon::Cannon(GameState* gameState, std::ifstream& in, Texture* texture, float velocity, PlayState* myPlayState) : SceneObject(gameState, myPlayState, in, texture),
	_velocity(velocity)
{
	in >> _shootCD;
