...
public:
	Alien(Point2D<float> pos, Texture* texture, std::pair<uint, uint> size, GameState* game, PlayState* myPlayState, SDLApplication* app, float velocity,
		int type, bool idle);
	Alien(GameState* game, PlayState* myPlayState, SDLApplication* app, std::ifstream& in, Texture* texture, uint velocity, int lastestRow);
	~Alien();
