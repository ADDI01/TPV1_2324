...
public:
	Laser(Point2D<float> pos, Vector2D<float> velocity, std::pair<uint, uint> size, GameState* game, PlayState* myPlayState, SDLApplication* app,
		SDL_Renderer* myRenderer, Father father);
	Laser(GameState* game, PlayState* myPlayState, SDLApplication* app, SDL_Renderer* renderer, std::ifstream& in, std::pair<uint, uint> size, Vector2D<float> velocity);
	~Laser();
